...
local function inspectEntry(plan, ir, entry, opts)
    ...
    if options.policy_kind == "payload" then
        if hasPostfix(entry) or hasPayloadBindings(entry) then
    ...
    if softHomingEnabled(options) then
        ...
        local soft_requested = options.force_soft_homing_enabled == true
            or options.soft_homing_probe == true
        if soft_requested and count > 1 then
...
local function launchPayloadForSpec(launch_spec, event_context, opts)
    ...
    if payload.homing_target_position == nil and payload.hit_object == nil and payload.homing_target_object == nil then
        payload.homing_target_position = (opts and opts.homing_target_position) or fallbackTargetPosition(payload)
        payload.homing_target_id = payload.homing_target_id or (opts and opts.homing_target_id) or "homing_policy_fallback_target"
    end
...
function homing_launch_policy.applyToLaunchSpec(plan, ir, entry, launch_spec, event_context, opts)
    ...
    log.info(string.format(
        "SPELLFORGE_HOMING_POLICY_APPLIED recipe_id=%s event_kind=%s slot_id=%s homing_mode=%s homing_field=%s target_provider=%s",
