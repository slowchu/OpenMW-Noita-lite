...
local function runSmoke()
    ...
    waitFor(state.pending_probe, request_id, 5, function(result)
        ...
        assertLine(result.user_data_routing_ok == true, "Spellforge userData routing is preferred when present")
        assertLine(result.user_data_mismatch_guard_ok == true, "userData/helper metadata mismatch fails safely", result.user_data_mismatch_error)
