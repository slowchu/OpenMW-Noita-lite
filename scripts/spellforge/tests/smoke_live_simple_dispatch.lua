...
local live_dispatch_recipes_fixture = require("scripts.spellforge.tests.fixtures.live_dispatch_recipes")
...
local function fixtureRecipeCount(fixture)
    local count = 0
    for key, value in pairs(fixture or {}) do
        if type(key) == "string" and key:match("_TARGET$") and type(value) == "table" then
            count = count + 1
        end
    end
    return count
...
local function runIrRuntimeAdapterStatusSmoke()
    ...
    local function finish()
        requestRuntimeStats(false, function(stats_result)
            ...
            log.info(string.format(
                "SPELLFORGE_SMOKE_FIXTURE_LOAD_OK live_dispatch_recipes=true live_dispatch_recipe_count=%s",
                tostring(fixtureRecipeCount(live_dispatch_recipes_fixture))
            ))
